...
PROCESS_MODEL_VERSION = "cotton_process_v11_strong_env_dnn_process_cnn"
...
# V5 strong-environment-DNN + temporal-process-CNN normalization.
...
class HybridBackbone(nn.Module):
    """
    Strong DNN + Temporal CNN + Static DNN
    """
    ...
    def __init__(
        ...
        self.register_buffer(
            "sequence_mean",
            torch.as_tensor(sequence_mean)
        )
        self.register_buffer(
            "sequence_std",
            torch.as_tensor(sequence_std)
        )
        self.register_buffer(
            "static_mean",
            torch.as_tensor(static_mean)
        )
        self.register_buffer(
            "static_std",
            torch.as_tensor(static_std)
        )

        # Strong DNN temporal encoder
        self.sequence_dnn = nn.Sequential(
            nn.Linear(sequence_dim, hidden),
            nn.SiLU(),
            nn.Linear(hidden, hidden),
            nn.SiLU(),
        )
    ...
    def forward(self, sequence, static):
        ...
        h_dnn = self.sequence_dnn(sequence)

        h_cnn = self.sequence_cnn(
            sequence.transpose(1,2)
        )

        h_cnn = h_cnn.transpose(1,2)
        ...
        h = self.temporal_fusion(h)

        h_static = self.static_net(static)
...
def make_backbone(kind, sequence_dim, static_dim, hidden, sequence_mean, sequence_std, static_mean, static_std):
    if kind == "dnn":
        backbone_class = DNNBackbone
    elif kind in ("cnn", "temporal_cnn"):
        backbone_class = TemporalBackbone
    elif kind == "hybrid":
        backbone_class = HybridBackbone
    else:
...
class ProcessConstrainedNN(nn.Module):
    ...
    those daily states with a 1D CNN before fusion for yield prediction.
    """
    ...
    def __init__(
        ...
        #
        # The differentiable process and its 1D CNN encoder are unchanged.
        ...
        # CNN reads four DAILY PROCESS STATE channels along the time axis.
        # Input shape before Conv1d: [B, 4, T].
    ...
    def forward(self, sequence, static, planting_day, soil_capacity, initial_soil_water):
        ...
        # =================================================
        # V4-smoke: differentiable DAILY process-state representation
        #
        # z_t = [relative SW_t, normalized ET_t, F_W,t, normalized ΔB_t]
        # shape before CNN: [B, T, 4]
        # Conv1d input:      [B, 4, T]
        #
        # Important:
        # - raw irrigation is NOT a CNN input channel;
        # - irrigation influences these states through the explicit crop-water
        #   recurrence, preserving I_t -> process states -> CNN -> yield.
        ...
        # Conv1d operates over time: [B, channels=4, T].
        ...
        # =================================================
        # V5 strong environment DNN + differentiable process CNN -> yield
        # =================================================
...
def parse_args():
    ...
    train_parser.add_argument("--backbone", choices=("cnn", "dnn", "hybrid"), default="dnn")
