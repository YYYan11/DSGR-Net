...
PROCESS_MODEL_VERSION = "cotton_process_v11_strong_env_only_ablation"
...
class ProcessConstrainedNN(nn.Module):
    ...
    def forward(self, sequence, static, planting_day, soil_capacity, initial_soil_water):
        ...
        # Matched ablation: keep the V5 strong environment encoder and the
        # exact same 64->64->hidden->1 fusion head, but remove all process-CNN
        # information from the yield path. The second half is fixed to zero.
        # Thus yield depends only on the irrigation-masked strong DNN branch.
        process_embedding_for_yield = torch.zeros_like(process_embedding)
        fusion = torch.cat(
            (
                environment_embedding,
                process_embedding_for_yield,
            ),
            dim=1,
        )
        ...
        return {
            ...
            "process_embedding_for_yield": process_embedding_for_yield,
