...
class ProcessConstrainedNN(nn.Module):
    ...
    def __init__(
        ...
        self.process_summary_projection = nn.Sequential(
            nn.Linear(3, hidden),
            nn.SiLU(),
        )

        self.process_gate = nn.Sequential(
            nn.Linear(hidden * 2, hidden),
            nn.SiLU(),
            nn.Linear(hidden, 1),
        )

        # Start exactly from the original V5 fusion:
        # process scale = 1 + 0.25 * tanh(0) = 1.
        nn.init.zeros_(self.process_gate[-1].weight)
        nn.init.zeros_(self.process_gate[-1].bias)
    ...
    def forward(self, sequence, static, planting_day, soil_capacity, initial_soil_water):
        ...
        process_gate = torch.tanh(
            self.process_gate(gate_input)
        )

        # Residual process scaling around the original V5 value (= 1.0).
        process_scale = 1.0 + 0.25 * process_gate

        process_embedding_gated = (
            process_scale * process_embedding
        )
